from eddsa.algorithms.eddsa import EdDSA
...
class Ed25519(EdDSA):
    ...
    def sign(message: bytes, keypair: Keypair) -> bytes:
        ...
        # 2. Compute the nonce
        r = int.from_bytes(sha512(prefix + message), byteorder='little')
        ...
        k = int.from_bytes(sha512(R + keypair.public_bytes() + message), byteorder='little')
    ...
    def verify(signature: bytes, message: bytes, public_key: bytes) -> bool:
        ...
        try:
            ...
            k = int.from_bytes(sha512(signature[:32] + public_key + message), byteorder='little')
            k = scalar_ops.reduce(k)
